import { publicSnapshotFetch } from "./public-snapshot";
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  const response = await publicSnapshotFetch(path, { signal: !init?.method || init.method === "GET" ? AbortSignal.timeout(45000) : undefined, ...init, headers: { "content-type": "application/json", ...(init?.headers || {}) } });
...
export const api = {
  ...
  artifactJson: (jobId: string, name: string) => request<unknown>(`/api/jobs/${encodeURIComponent(jobId)}/artifacts/${encodeURIComponent(name)}`),
  artifacts: async (id: string) => (await request<{ artifacts: JobArtifact[] }>(`/api/jobs/${encodeURIComponent(id)}/artifacts`)).artifacts,
  artifactUrl: (jobId: string, name: string) => `/real-apps/geo/seed/jobs/${encodeURIComponent(jobId)}/${encodeURIComponent(name)}`,
