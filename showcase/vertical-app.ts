...
export function verticalAppDocument(key: LabKey, dataset: DemoDataset, mode: DemoMode, report: unknown, instanceId: string) {
  const meta = verticalAppNames[key];
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeText(meta.title)}</title><style>${verticalStyle}</style></head><body data-app="${key}"><main id="app"><header class="app-header"><div><small>PAW / ${key.toUpperCase()}</small><h1>${meta.title}</h1><p>${meta.subtitle}</p></div><div class="app-actions"><button data-action="export-work">导出工作记录</button><button data-action="play">播放示例</button></div></header><div class="app-meta"><span>${dataset.records.length} 条记录</span><span>${dataset.cases.length} 个检查案例</span><span>公开示例 · 浏览器内执行</span><span id="storage-status">当前窗口</span></div><div id="workspace"></div><p id="notice" role="status" aria-live="polite"></p><footer>业务写入只作用于演示副本。知识台执行本地检索与原文摘录，诊断台执行明确规则；不调用模型、生产业务系统或个人记忆。</footer></main><script>const execute=${runLabDemo.toString()};const records=${safeJson(dataset.records)};const cases=${safeJson(dataset.cases)};const mode=${safeJson(mode)};const key=${safeJson(key)};const start=${mountVerticalApp.toString()};start({execute,records,cases,mode,key,report:${safeJson(report)},instanceId:${safeJson(instanceId)}});</script></body></html>`;
}

type VerticalContext = { execute: typeof runLabDemo; records: DemoDataset['records']; cases: DemoDataset['cases']; mode: DemoMode; key: LabKey; report: unknown; instanceId: string };
function mountVerticalApp(ctx: VerticalContext) {
  ...
  type State = { assignments: Record<string, string>; tickets: Record<string, string>; memories: Record<string, Row>; answers: { query: string; id: string; text: string }[]; actions: Action[] };
  ...
  const result = (id: string) => execute(key, records, id, mode);
  const fresh = (): State => ({ assignments: {}, tickets: {}, memories: {}, answers: [], actions: [] });
  const storageKey = `paw.vertical-work.v1:${instanceId}`;
  let persistent = false;
  let state = fresh();
  try {
    const saved = JSON.parse(localStorage.getItem(storageKey) || 'null');
    if (saved && typeof saved === 'object' && saved.assignments && saved.tickets && saved.memories && Array.isArray(saved.answers) && Array.isArray(saved.actions)) state = saved;
  ...
  const source = (row: Row) => `<section class="source" aria-label="原始来源"><small>${origin(row)}</small><h3>${escape(row.sourceTitle || row.title || '示例来源')}</h3><p>${escape(row.text || label(row))}</p><code>${escape(row.sourceId || row.id)}</code></section>`;
  ...
  function render() {
    ...
    if (key === 'enterpriseops') {
      $('#workspace').innerHTML = `<div class="workspace">${list(records)}<section class="detail">${row ? `<header class="detail-title"><small>客户负责人变更</small><h2>${escape(row.customer || row.id)}</h2><span class="badge ${result(selected).value === '允许' ? 'ok' : 'no'}">${result(selected).value}</span></header><div class="handover"><div><small>当前负责人</small><strong data-current-owner>${escape(state.assignments[selected] || row.currentOwner || '原负责人')}</strong></div><span>→</span><div><small>拟交接给</small><strong>${escape(row.candidateOwner || row.id)}</strong></div></div><h3>交接条件</h3><ul class="checks">${check(row.region === 'CN', '地区一致', `目标地区 CN / 当前 ${row.region}`)}${check(row.role === 'manager', '角色匹配', `需要 manager / 当前 ${row.role}`)}${check(Number(row.tenureDays) >= 90, '任期要求', `至少 90 天 / 当前 ${row.tenureDays} 天`)}</ul><div class="action-row"><button class="primary" data-action="apply-assignment" ${result(selected).value === '允许' ? '' : 'disabled'}>执行交接</button><button data-action="undo" ${state.actions.length ? '' : 'disabled'}>撤销最近操作</button></div><p class="muted">更新当前演示中的负责人；条件不满足时不会写入。</p>${recentActions()}` : empty('选择一条客户交接请求')}</section></div>`;
    } else if (key === 'rag') {
  ...
  function search(input: string) { query = input.trim(); queryResult = execute('rag', records, query, mode); sourceOpen = ''; render(); }
  document.addEventListener('submit', event => {
    if ((event.target as HTMLElement).id === 'query-form') { event.preventDefault(); search((document.getElementById('query') as HTMLInputElement).value); }
  ...
  document.addEventListener('click', event => {
    ...
    if (name === 'browse-source') { const item = records.find(item => item.id === button.dataset.openSource); if (item) { queryResult = { value: String(item.id), detail: String(item.text) }; sourceOpen = String(item.id); render(); } }
    ...
    if (name === 'apply-assignment' && row && result(selected).value === '允许') {
      const before = state.assignments[selected] ?? null;
      state.assignments[selected] = String(row.candidateOwner || row.id); action('assignment', selected, before, `负责人：${before || row.currentOwner || '原负责人'} → ${state.assignments[selected]}`); render(); notify('交接已写入当前演示副本。');
    }
    ...
    if (name === 'accept-memory' && row) {
      const before = state.memories[selected] ?? null;
      if (result(selected).value === '保留') state.memories[selected] = { ...row }; else delete state.memories[selected];
      action('memory', selected, before, `${row.title || row.id}：${result(selected).value}`); render(); notify(result(selected).value === '保留' ? '长期记忆已更新，并保留原始来源。' : '已核对，本条不进入长期记忆。');
    }
  ...
  const steps: Step[] = key === 'rag' ? [
    { label: '提出问题', instruction: '搜索一个业务问题。', selector: `[data-example="${firstCase?.id}"]`, after: () => query === firstCase?.input && !!queryResult },
    ...
    { label: '整理记忆', instruction: '应用整理决定，保留来源。', selector: '[data-action="accept-memory"]', after: () => lastAction === 'memory' },
