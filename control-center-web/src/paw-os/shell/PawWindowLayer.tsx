...
function layoutRoomCollaborationFocus(
  ...
  // Size reading windows for a useful transcript, not just the generic resize
  // floor. When a grid would squeeze text, keep a wider scrollable partner row.
  const rowsAvailable = Math.max(1, Math.floor((height + gap) / (280 + gap)));
  const columns = Math.ceil(partners.length / rowsAvailable);
  const partnerWidth = Math.min(520, (width - 640 - columns * gap) / columns);
  if (partnerWidth >= 320) {
    ...
    const partnerHeight = Math.max(PAW_WINDOW_MIN_HEIGHT, Math.min(320, Math.round(height * .42)));
    ...
    const railWidth = Math.min(420, width);
    partners.forEach((node, index) => frames.set(node.id, {
