...
export function pawFocusWindowLayerSize(): { width: number; height: number } {
  const width = typeof window === 'undefined' ? 1280 : window.innerWidth;
  const height = typeof window === 'undefined' ? 800 : window.innerHeight;
  const director = typeof document === 'undefined' ? null : document.querySelector('.paw-room-flow-showcase');
  const usableBottom = director ? Math.min(height, director.getBoundingClientRect().top) : height;
  return {
    width: Math.max(PAW_WINDOW_MIN_WIDTH, width),
    height: Math.max(PAW_WINDOW_MIN_HEIGHT, usableBottom - PAW_MENU_BAR_HEIGHT),
  };
}
