...
import { TraceShowcaseWorkbench } from '@/features/trace-agent/showcase-workbench';
...
export default function PawTraceAgentApp({ initialRoute = '/trace-agent' }: { initialRoute?: string }) {
  return <div className="paw-trace-agent-app" data-app-id="trace-agent"><MemoryRouter initialEntries={[initialRoute || '/trace-agent']}><TraceRouteBridge initialRoute={initialRoute || '/trace-agent'} />{isPawTraceFlowShowcase() ? <TraceShowcaseWorkbench /> : <TraceAgentFeature />}</MemoryRouter></div>;
}
