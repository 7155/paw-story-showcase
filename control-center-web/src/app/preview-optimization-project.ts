...
export class PreviewOptimizationProject {
 private frozen=''; private compared=''; private selected:RepairVariant='repaired'; private adopted='';
 private supportedMaterialContents='';
 private suites:ReturnType<typeof evaluateRepairSuite>[]=[];
 ...
 private signature(project:LabProject){return `${this.conditions(project)}:${project.artifacts.find(a=>a.artifactId==='repair-config')?.revision}`;}
 ...
 status(project:LabProject){return {hasRun:project.artifacts.some(a=>a.artifactId==='repair-file'),hasReport:project.artifacts.some(a=>a.artifactId==='repair-checks'),frozen:this.frozen===this.conditions(project),current:this.compared===this.signature(project),canKeep:this.compared===this.signature(project)&&this.suites.find(s=>s.variant===this.selected)?.decision==='keep',adopted:this.adopted===this.signature(project),selected:this.selected};}
 ...
 command(project:LabProject,input:Record<string,unknown>,put:Publish,read:(id:string)=>LabArtifact|undefined){
  ...
  if(op==='optimization_freeze') {this.assertSupportedInputs(project,read);this.frozen=this.conditions(project);return put(project,{artifactId:'repair-freeze',title:'已冻结检查条件',kind:'evidence',view:'markdown',summary:'八项内置条件已冻结，可以执行同条件比较',content:`# 已冻结 8 项内置检查条件\n\n材料版本：${project.materialSetId}\n\n实际执行内置模拟文件与八项故障，逐题核对有效文件、业务写入次数、登记次数、回执状态和恢复路径。\n\n此演示不执行自定义材料或改写的检查标准；恢复初始材料和内置题目后可以重新冻结。候选配置变化后需要重新比较。`});}
  ...
  if(op==='optimization_compare') {
   if(this.frozen!==this.conditions(project))throw new Error('请先冻结当前材料与八项检查条件。');
   this.assertSupportedInputs(project,read);
   this.suites=repairVariants.map(v=>evaluateRepairSuite(v.id));this.compared=this.signature(project);
   put(project,{artifactId:'repair-checks',title:'逐题差异与失败原因',kind:'evaluation',view:'table',summary:'逐题对照三个候选的文件、计数、回执和恢复行为',content:{columns:[{key:'case',label:'复测条件'},{key:'baseline',label:'原流程'},{key:'cheap',label:'候选 A'},{key:'repaired',label:'候选 B'},{key:'difference',label:'具体差异'}],rows:repairCases.map((c,i)=>({case:c.title,baseline:this.suites[0].runs[i].passed?'通过':'失败',cheap:this.suites[1].runs[i].passed?'通过':'失败',repaired:this.suites[2].runs[i].passed?'通过':'失败',difference:this.suites.map((s,j)=>`${repairVariants[j].name}：${s.runs[i].checks.filter(k=>!k.passed).map(k=>`${k.label}（${k.observed}）`).join('；')||'检查全部通过'}`).join('\n')})),caption:'每列使用相同八种条件、相同初始文件与判定函数。没有修改 Gold 来提高候选得分。'}});
   return put(project,{artifactId:'repair-compare',title:'同条件候选比较',kind:'evaluation',view:'table',summary:this.suites.map((s,i)=>`${repairVariants[i].name} ${s.passed}/${s.total}`).join(" · "),content:{columns:[{key:'candidate',label:'候选'},{key:'change',label:'改了什么'},{key:'quality',label:'任务与恢复门槛'},{key:'decision',label:'本轮判定'},{key:'cost',label:'虚构费率下的演示估算'},{key:'boundary',label:'适用边界'}],rows:this.suites.map((s,i)=>({candidate:repairVariants[i].name,change:repairVariants[i].change,quality:`${s.passed}/${s.total}`,decision:s.decision.toUpperCase(),cost:`$${s.estimate.toFixed(6)} · 非账单`,boundary:repairVariants[i].risk})),caption:'先比较业务质量；token 数按示例文本长度估算，费率为显式虚构值。每题报告保留公式，不是 Provider usage、真实延迟或生产节省。'}});
  }
  if(op==='optimization_keep') {
   if(this.compared!==this.signature(project))throw new Error('当前条件或配置已变化；旧报告可查看，但不能作为当前采用依据。请重新比较。');
   const suite=this.suites.find(s=>s.variant===selected)!;if(suite.decision!=='keep')throw new Error(`当前候选仅通过 ${suite.passed}/${suite.total}，仍有失败项，不能采用。`);
   this.adopted=this.signature(project);return put(project,{artifactId:'repair-decision',title:'候选采用回执',kind:'evidence',view:'markdown',summary:'候选 B 已在当前冻结条件下选用',content:`# 已选择 ${repairVariants.find(v=>v.id===selected)!.name}\n\n当前八项合成复测全部通过。原流程及候选 A 的失败报告保留，不覆盖历史。\n\n## 采用范围\n\n仅本演示的当前材料与配置。未修改真实 Pi/PAW 源码，未证明真实文件并发、网络时序、Provider 质量或生产成本。\n\n## 下一次遇到登记失败\n\n先回读文件与 operationId；保留有效内容，只恢复辅助登记。未知回执仍需核对。\n\n## 变更之后\n\n修改材料、检查条件或已保存配置后，此回执不再代表新版本的验证。`});
  }
