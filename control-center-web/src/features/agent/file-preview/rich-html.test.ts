...
describe('rich HTML preview', () => {
  ...
  it('round trips unicode reports larger than 3 MB through bounded chunks', () => {
    const source = '研究报告 🚀 数据流 '.repeat(400_000);
    expect(new TextEncoder().encode(source).byteLength).toBeGreaterThan(3 * 1024 * 1024);

    const chunks = richHtmlPreviewChunks(source);
    expect(chunks.length).toBeGreaterThan(90);
    expect(chunks.join('')).toBe(source);
    expect(chunks.every((chunk) => chunk.length <= 0x8000)).toBe(true);
    expect(richHtmlPreviewUrl()).toBe(`${import.meta.env.BASE_URL}__paw_lab_preview.html`);
  });

  it('rejects a source beyond the preview transfer bound', () => {
    expect(() => richHtmlPreviewChunks('x'.repeat(RICH_HTML_PREVIEW_MAX_CHARS + 1))).toThrow(RangeError);
  });
});
