...
function trialSuite(caseCount = 16, jobs: GoldenJob[] = []): GoldenSuite {
  const development = Array.from({ length: caseCount }, (_, index) => ({
    ...goldenCase(`case-dev-${index + 1}`), question: `开发问题 ${index + 1}`,
    review: { status: 'approved' as const, note: '', reviewedAtMs: 3000 },
  }));
  return suite({
    cases: [...development, { ...goldenCase('case-hold', 'holdout'), question: '留出问题，不应出现在试跑选择器' }],
    calibration: { calibrationId: 'cal-ready', suiteRevision: 1, judgeConfig: model, judgments: [], metrics: { total: 1, comparable: 1, agreement: 1, falsePasses: 0, falseFails: 0, uncertain: 0 }, ready: true, reasons: [], createdAtMs: 6000 },
    snapshot: { snapshotId: 'snapshot-1', suiteId: 'suite-1', version: 1, sourceRevision: 1, createdAtMs: 6000, developmentCount: caseCount, holdoutCount: 1, judgeConfig: model },
    jobs,
  });
}

function developmentTrialResult(): ExperimentResult {
  const base = experimentResult();
  return {
    ...base,
    referenceAuthority: 'agent_assisted',
    runMode: 'development_trial',
    caseSelection: { split: 'development', caseIds: ['case-dev'], selectedCount: 1, frozenDevelopmentCount: 16, holdoutConsumed: false, paidCallPlan: { answerCalls: 2, planningCalls: 0, judgeCalls: 2, optimizationCalls: 0, holdoutCalls: 0, totalCalls: 4, basis: 'one baseline and one candidate over selected development cases' } },
    holdout: null,
    optimization: { enabled: false, maxCandidates: 1, selectedCandidateIndex: 0, proposals: [] },
    comparison: { ...base.comparison, decision: 'inconclusive', holdoutDelta: null, evidenceScope: 'development_only', reasons: ['仅完成开发题试跑，尚未验证留出题。'], improvementBasis: null },
  };
}

describe('Golden workflow user boundaries', () => {
  it('keeps the original calibration and cost visible beside a free quote recheck', async () => {
    const item = goldenCase(); item.review.status = 'approved';
    const original: NonNullable<GoldenSuite['calibration']> = {
      calibrationId: 'calibration-original', suiteRevision: 1, judgeConfig: model,
      metrics: { total: 12, comparable: 12, agreement: .75, falsePasses: 0, falseFails: 0, uncertain: 3 },
      ready: false, reasons: [], createdAtMs: 1000,
      judgments: [{ caseId: item.caseId, sampleId: item.samples[0].sampleId, verdict: 'uncertain', reason: '原引文无法逐字对应。', evidence: [] }],
    };
    const recalibrated = { ...original, calibrationId: 'calibration-derived', ready: true,
      metrics: { ...original.metrics, agreement: 1, uncertain: 0 },
      reprocessedFrom: { jobId: 'job-original', calibrationId: original.calibrationId, suiteRevision: 1, judgeProtocolVersion: 'v3' },
      judgments: [{ ...original.judgments[0], verdict: 'pass' as const, reason: '已对应原文。', evidence: item.evidence,
        quoteNormalizations: [{ evidenceIndex: 0, sourceId: 'source-1', from: '退款期限为\n七天。', to: '退款期限为七天。', startChar: 0, endChar: 8, offsetUnit: 'Unicode', operations: [], alignmentPolicyVersion: 'paw.golden.quote-alignment.v1' }] }],
    };
    const current = suite({ cases: [item], calibration: recalibrated, jobs: [
      { ...job('completed', 'calibrate'), jobId: 'job-original', result: { calibration: original, usage: { ...usage, calls: 12, estimatedCostUsd: .1162192 } } },
      { ...job('completed', 'calibrate'), jobId: 'job-derived', result: { calibration: recalibrated, usage: { calls: 0, source: 'read_only_reprocess' } } },
    ] });
    render(<CalibrationPanel suite={current} disabled={false} onLabel={vi.fn(async () => true)} onJudge={vi.fn(async () => true)} onCalibrate={vi.fn()} onNext={vi.fn()} />);
    expect(screen.getByText(/本次新增模型调用：0/)).toBeVisible();
    expect(screen.getByText(/原校准一致率 75%，无法判定 3 个/)).toBeVisible();
    expect(screen.getByText(/原调用价表估算：\$0.1162192；实际账单：未提供/)).toBeVisible();
    const user = userEvent.setup();
    await user.click(screen.getByText('查看原始判定'));
    expect(screen.getByText(/原引文无法逐字对应/)).toBeVisible();
    await user.click(screen.getByText('参考标签：未标注 · 评审：通过'));
    await user.click(screen.getByText('查看 1 处引用格式对齐'));
    expect(screen.getByText('原模型引文')).toBeVisible();
    expect(screen.getByText('对应原文连续片段')).toBeVisible();
  });
  ...
  it('offers immutable full corpus supplementation beside an omitted citation and keeps it a separate save', async () => {
    const item = goldenCase();
    item.evidence = [{ sourceId: 'source-1', quote: '全文中才有的精确句子。' }];
    const onSupplement = vi.fn().mockResolvedValue(true);
    render(<CaseReview suite={suite({ knowledge: { projectId: 'project-1', indexId: 'index-1', corpusHash: 'corpus-1', documentCount: 1, chunkCount: 1, profile: {} }, cases: [item] })} disabled={false} onReview={vi.fn(async () => true)} onSupplement={onSupplement} onNext={() => {}} />);
    await userEvent.setup().click(screen.getByRole('button', { name: '从绑定全文补充引用' }));
    await waitFor(() => expect(onSupplement).toHaveBeenCalledWith({ caseId: 'case-dev', evidenceIndex: 0, sourceId: 'source-1', quote: '全文中才有的精确句子。' }));
    expect(screen.getByRole('textbox', { name: '引用 1 的原文' })).toHaveValue('全文中才有的精确句子。');
  });
  ...
  it('offers completed calibration quote reprocessing as a formatting-only no-call action', async () => {
    let current = suite({ jobs: [{ ...job('completed', 'calibrate'), canReprocessCalibration: true }] });
    const commands: GoldenCommand[] = [];
    const transport = new MockControlTransport({ routes: {
      'agent.eval-lab.golden.get': () => read(current),
      'agent.eval-lab.golden.command': (request: ControlRequest) => {
        ...
        current = suite({ jobs: [{ ...job('queued', 'calibrate'), reprocessOnly: true, reprocessSourceJobId: 'job-calibrate' }] });
        return receipt(command, current);
      },
    } });
    mount(transport);
    const user = userEvent.setup();
    await enabledButton('重新核对引用');
    expect(screen.getByText('仅对齐引用中的格式差异，不会重新调用模型；原校准结果和模型回执会保留。')).toBeInTheDocument();
    await user.click(screen.getByRole('button', { name: '重新核对引用' }));
    expect(commands).toHaveLength(1);
    expect(commands[0]).toMatchObject({ action: 'reprocess_calibration', suiteId: 'suite-1', expectedRevision: 1, input: { jobId: 'job-calibrate' } });
    expect(transport.requests.filter(({ request }) => request.pathId === 'agent.eval-lab.golden.command')).toHaveLength(1);
  });

  it('distinguishes paid draft completion from cached reprocessing and preserves its nonce after response loss', async () => {
    let current = suite({ jobs: [{ ...job('failed'), canReprocess: true, canCompleteDraft: true, error: '只有32/48题' }] });
    const commands: GoldenCommand[] = [];
    const fetchMock = vi.fn(async (_url: RequestInfo | URL, init?: RequestInit) => {
      if (init?.method !== 'POST') return json(read(current));
      const command = JSON.parse(String(init.body)) as GoldenCommand; commands.push(command);
      current = suite({ jobs: [job('running')] });
      if (commands.length === 1) throw new TypeError('lost completion admission');
      return json(receipt(command, current, true));
    });
    mount(new HttpControlTransport({ baseUrl: 'http://127.0.0.1:8791', fetch: fetchMock }));
    const user = userEvent.setup();
    await enabledButton('补齐题目并校验引用');
    expect(screen.getByText('复用原结果重新处理，不会重新调用模型。')).toBeVisible();
    expect(screen.getByText(/补齐会复用已返回内容，并调用模型/)).toBeVisible();
    await user.click(screen.getByRole('button', { name: '补齐题目并校验引用' }));
    await user.click(await screen.findByRole('button', { name: '核对本次操作' }));
    await waitFor(() => expect(commands).toHaveLength(2));
    expect(commands[0]).toMatchObject({ action: 'resume', input: { jobId: 'job-draft', completeDraft: true } });
    expect(commands[1]).toEqual(commands[0]);
  });
...
describe('Golden development trial', () => {
  it('selects a bounded development batch and submits no holdout identifiers', async () => {
    const onExperiment = vi.fn(async (_input: GoldenCommand['input']) => true);
    render(<GoldenExperiment suite={trialSuite()} disabled={false} onFreeze={vi.fn()} onExperiment={onExperiment} />);
    expect(screen.getByRole('status', { name: '开发试跑计划' })).toHaveTextContent('12 / 16 道开发题已选择');
    expect(screen.getByRole('status', { name: '开发试跑计划' })).toHaveTextContent('计划模型调用：48 次');
    expect(screen.queryByText('留出问题，不应出现在试跑选择器')).not.toBeInTheDocument();
    await userEvent.setup().click(screen.getByRole('button', { name: '开始开发试跑' }));
    await waitFor(() => expect(onExperiment).toHaveBeenCalledTimes(1));
    expect(onExperiment).toHaveBeenCalledWith(expect.objectContaining({
      snapshotId: 'snapshot-1', runMode: 'development_trial', optimizePrompt: false, maxCandidates: 1,
      caseIds: Array.from({ length: 12 }, (_, index) => `case-dev-${index + 1}`),
    }));
    expect(onExperiment.mock.calls[0][0]).not.toHaveProperty('holdout');
  });

  it('blocks an empty or over-limit development selection in the client', async () => {
    render(<GoldenExperiment suite={trialSuite()} disabled={false} onFreeze={vi.fn()} onExperiment={vi.fn(async () => true)} />);
    const user = userEvent.setup();
    for (const checkbox of screen.getAllByRole('checkbox', { name: /选择开发题/u }).slice(0, 12)) await user.click(checkbox);
    expect(screen.getByRole('status', { name: '开发试跑计划' })).toHaveTextContent('0 / 16 道开发题已选择');
    expect(screen.getByRole('button', { name: '开始开发试跑' })).toBeDisabled();
    expect(screen.getByRole('alert')).toHaveTextContent('至少选择 1 道');
  });

  it('renders development trial evidence without manufacturing a holdout report', () => {
    const result = developmentTrialResult();
    expect(isExperimentResult(result)).toBe(true);
    expect(isExperimentResult({
      ...result,
      caseSelection: { ...result.caseSelection!, caseIds: ['case-missing'], paidCallPlan: { ...result.caseSelection!.paidCallPlan!, totalCalls: 3 } },
      comparison: { ...result.comparison, decision: 'improved', evidenceScope: 'development_and_holdout' },
    })).toBe(false);
    render(<GoldenExperiment suite={trialSuite(16, [job('completed', 'experiment', result)])} disabled={false} onFreeze={vi.fn()} onExperiment={vi.fn(async () => true)} />);
    expect(screen.getByText('开发题小批试跑')).toBeInTheDocument();
    expect(screen.getByText(/Agent 辅助标注的冻结标准/)).toBeInTheDocument();
    expect(screen.getByText(/本次执行 1 \/ 16 道已冻结开发题/)).toBeInTheDocument();
    expect(screen.getByText('证据不足，暂不能判断最终改善')).toBeInTheDocument();
    expect(screen.getByRole('region', { name: '本次开发题比较' })).toBeInTheDocument();
    expect(screen.queryByRole('region', { name: '留出题比较' })).not.toBeInTheDocument();
    expect(screen.queryByText('留出问题，不应出现在试跑选择器')).not.toBeInTheDocument();
  });
});
