...
export function LabAppDelivery({ projectId, preparing = false, onPrepare }: { projectId: string; preparing?: boolean; onPrepare?: (directory: string, appId?: string) => Promise<boolean> }) {
  ...
  return <section className="lab-app-delivery" aria-label="项目应用交付">
    ...
    {onPrepare && !projectId.startsWith('lab-showcase-') ? <details className="lab-app-delivery__sources"><summary>从执行目录准备应用</summary>
    ...
    {!catalog.isPending && !catalog.isError && !catalog.data?.items.length ? <div className="lab-project-empty"><PackageCheck size={28} /><h3>还没有准备好的应用版本</h3><p>让项目 Agent 在执行目录中实现应用并准备版本。界面、方法和输入由项目决定；准备完成后，就可以在这里试用和导出。</p></div>
      : app && version ? <><div className="lab-app-delivery__toolbar">
        ...
        {!offlineDemo ? <Button disabled={busy || app.activeVersion === version.version} onClick={() => void act({ action: 'activate', appId, expectedRevision: app.revision, clientRequestId: `app-activate:${crypto.randomUUID()}`, input: { version: version.version } })}><PackageCheck size={15} />{app.activeVersion === version.version ? '已添加至 PAW' : app.activeVersion ? '将此版本用于 PAW' : '添加至 PAW'}</Button> : null}
        ...
        {!offlineDemo ? <Button disabled={busy} onClick={() => void download('paw')}><Download size={15} />下载 PAW 应用包</Button> : null}
      ...
      <p className="lab-app-delivery__boundary">{offlineDemo ? '本轮生成的是离线演示 App，使用与测评相同的数据和规则。下载 ZIP 后直接打开 index.html，无需 Key 或服务；包内包含数据、源码和本轮测评回执。' : 'PAW 使用此版本的方法和已配置模型。独立 App 使用 Python 标准库运行；生成回答需要在目标环境配置兼容的模型服务，应用包不携带登录凭据。'}</p>
