...
import { RICH_HTML_PREVIEW_PATH } from '@/features/agent/file-preview/rich-html';
...
export function LabAppPreview({ app, version, calls, onActivity }: { app: LabApp; version: LabAppVersion; calls: LabAppCall[]; onActivity: () => void }) {
  ...
  const offlineDemo = version.spec.model.provider === 'offline-showcase';
  const frameUrl = offlineDemo
    ? previewUrl.replace(`${RICH_HTML_PREVIEW_PATH}#`, './__paw_lab_preview.html#') : previewUrl;
      ...
      if (data.kind === 'paw.lab-app.history') {
        const result = calls.filter((call) => call.appId === app.appId)
          ...
          .map((call) => ({ requestId: call.callId, version: call.version, actionId: call.actionId,
            input: call.input, state: call.state, result: call.result, message: call.error, progress: call.progress,
  ...
  }, [app.appId, app.revision, version.spec.actions, version.version, calls, send, onActivity]);
  useEffect(() => {
  ...
  return <div className="lab-app-preview">
    ...
    <iframe key={frameUrl} ref={frame} title={`${version.spec.title} · 应用预览`} hidden={workspaceVisible} sandbox={`allow-scripts allow-popups allow-popups-to-escape-sandbox${offlineDemo ? ' allow-forms' : ''}`} referrerPolicy="no-referrer" src={frameUrl} />
    {external && workspaceOpened ? workspaceUrl
      ? <iframe title={external.title} hidden={!workspaceVisible} sandbox="allow-scripts allow-same-origin allow-forms allow-downloads allow-popups allow-popups-to-escape-sandbox" referrerPolicy="no-referrer" src={workspaceUrl} />
      : <p hidden={!workspaceVisible} className="lab-project-error" role="alert">工作台地址不可用。请检查应用声明的 HTTPS 或本机服务地址；不能嵌入当前控制服务。</p> : null}
