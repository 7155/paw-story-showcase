...
import { parseKnowledgeState, type KnowledgeEvaluation, type KnowledgeState } from './knowledge-types';
import { baselineForCorpus } from './knowledge-baseline';
import type { JsonValue, LabArtifact, LabProject, ProjectReceipt } from './types';
...
function mount(read: () => unknown, onCommand = vi.fn(async (_input: Record<string, JsonValue>) => undefined as ProjectReceipt | undefined), options: {artifact?:LabArtifact;job?:Record<string,unknown>} = {}) {
  const current=options.artifact ? {...project,artifacts:[options.artifact],artifactCount:1} : project;
  const client = new QueryClient({ defaultOptions: { queries: { retry: false }, mutations: { retry: false } } }); clients.push(client);
  const transport = new MockControlTransport({ routes: { 'agent.eval-lab.projects.get': () => ({ ok: true, items: [current], project:current, supportedViews: [], knowledge: read(),...(options.artifact ? {artifact:options.artifact} : {}) }), 'agent.eval-lab.trials.get':{schemaVersion:'rag-ime.agent-lab-trial.v1',job:options.job} } });
  ...
  render(<QueryClientProvider client={client}><ControlTransportProvider transport={transport}><LabKnowledge project={current} initialJobId={options.job?.jobId as string|undefined} initialBaseline={Boolean(options.artifact)} busy={false} onCommand={onCommand} onBind={onBind} onOpenBinding={onOpenBinding} /></ControlTransportProvider></QueryClientProvider>);
  return { onCommand, onBind, client, transport };
}

describe('Knowledge resource frontend', () => {
  it('carries the saved first-step baseline into the actual evaluation command',async()=>{
    const data=ready(),plan=baselineForCorpus(data);plan.profile={...plan.profile,topK:7,contextChars:9000};
    const artifact:LabArtifact={artifactId:'baseline',revision:2,title:'知识库基线',kind:'knowledge_baseline',view:'json',content:plan,summary:'',actions:[],templateRef:null,createdAtMs:1,updatedAtMs:2};
    const {onCommand}=mount(()=>data,undefined,{artifact});
    fireEvent.click(await screen.findByRole('button',{name:'评测'}));
    await waitFor(()=>expect(screen.getByRole('spinbutton',{name:'Top K'})).toHaveValue(7));
    fireEvent.click(screen.getByRole('button',{name:'运行开发集检索评测'}));
    await waitFor(()=>expect(onCommand).toHaveBeenCalledWith({operation:'evaluate',indexId:'index-1',datasetId:'dataset-1',profile:plan.profile,split:'development'}));
  });
  it('opens a specific older trial even when it is absent from the recent knowledge list',async()=>{
    const data=ready(),profile={mode:'lexical',topK:3,threshold:0,rerank:false,candidateDepth:40,contextChars:8000};
    const job={jobId:'old-trial',clientRequestId:'request',sceneId:'knowledge-resource',state:'completed',publicSpec:{projectId:project.projectId,operation:'evaluate',indexId:'index-1',datasetId:'dataset-1',profile},cancelRequested:false,progress:'done',sessions:[],result:{kind:'evaluation'},error:'',createdAtMs:1,updatedAtMs:1,resumeAvailable:false};
    const {onCommand}=mount(()=>data,undefined,{job});
    fireEvent.click(await screen.findByRole('button',{name:'评测'}));
    await waitFor(()=>expect(screen.getByRole('spinbutton',{name:'Top K'})).toHaveValue(3));
    fireEvent.click(screen.getByRole('button',{name:'运行开发集检索评测'}));
    await waitFor(()=>expect(onCommand).toHaveBeenCalledWith({operation:'evaluate',indexId:'index-1',datasetId:'dataset-1',profile,split:'development'}));
  });

  it('shows Chinese chunking strategies in the evaluation comparison rows', async () => {
    const evaluation = (jobId: string, strategy: string): KnowledgeEvaluation => ({
      jobId, indexId: jobId, corpusHash: 'corpus-hash', datasetId: 'dataset-1', datasetHash: 'dataset-hash', split: 'development',
      plannedCount: 20, evaluatedCount: 20, unlabeledCount: 0, holdoutUseNumber: 0,
      profile: { mode: 'lexical', topK: 8, threshold: 0, rerank: false, candidateDepth: 40, contextChars: 16000 },
      indexConfig: { chunking: { strategy, size: 900, overlap: 120 } },
      report: { metrics: { metrics: { mrr: 0.5, recallAtK: { '8': 0.5 }, precisionAtK: { '8': 0.5 }, f1AtK: { '8': 0.5 }, ndcgAtK: { '8': 0.5 } } }, costs: { meanRetrievalLatencyMs: 12, retrievalCalls: 20 }, receiptSha256: 'receipt' },
    });
    mount(() => ({ ...ready(), evaluations: [evaluation('fixed900', 'fixed'), evaluation('paper900', 'paper'), evaluation('other900', 'markdown')] }));
    fireEvent.click(await screen.findByRole('button', { name: '评测' }));
    expect(await screen.findByText('切片：固定长度 · 900/120')).toBeInTheDocument();
    expect(screen.getByText('切片：论文结构 · 900/120')).toBeInTheDocument();
    expect(screen.getByText('切片：Markdown 结构 · 900/120')).toBeInTheDocument();
  });
  ...
  it('imports an executor path through the resource operation without adding it to model context', async () => {
    ...
    const path = await screen.findByRole('textbox', { name: '执行器上的文件夹或文件路径' });
    ...
    await waitFor(() => expect(onCommand).toHaveBeenCalledWith({ operation: 'import_corpus', path: '/data/company-kb', fields: {}, cleaning: { removeRepeatedMargins: true, normalizeWhitespace: true } }));
  ...
  it('keeps the current path during a failed read and never resubmits a resource job', async () => {
    ...
    const input = await screen.findByRole('textbox', { name: '执行器上的文件夹或文件路径' });
  ...
  it('uploads every selected folder file with its relative name into one corpus',async()=>{
    vi.stubGlobal('crypto',webcrypto);const calls:Record<string,JsonValue>[]=[];let index=0;
    const onCommand=vi.fn(async(input:Record<string,JsonValue>):Promise<ProjectReceipt>=>{
      calls.push(input);if(input.operation==='upload_begin')index++;
      return {ok:true,project,clientRequestId:'upload',replayed:false,upload:{uploadId:`file-${index}`,chunkBytes:512*1024}};
    });
    mount(state,onCommand);
    await waitFor(()=>expect(screen.getByRole('button',{name:'连接已有知识库'})).not.toBeDisabled());
    const files=['a.txt','b.txt'].map(name=>{
      const bytes=new TextEncoder().encode(name),file=new File([bytes],name);
      Object.defineProperties(file,{arrayBuffer:{value:async()=>bytes.buffer},webkitRelativePath:{value:`papers/${name}`}});return file;
    });
    fireEvent.change(screen.getByLabelText('选择文件夹'),{target:{files}});
    fireEvent.click(screen.getByRole('button',{name:'整理资料'}));
    await waitFor(()=>expect(calls.at(-1)).toMatchObject({operation:'import_corpus',uploadIds:['file-1','file-2']}));
    expect(calls.filter(call=>call.operation==='upload_begin').map(call=>call.name)).toEqual(['papers/a.txt','papers/b.txt']);
  });
});
