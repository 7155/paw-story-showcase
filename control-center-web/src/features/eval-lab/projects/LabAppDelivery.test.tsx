...
describe('Lab application delivery', () => {
  it('describes adaptive research budgets and its required PAW gateway', async () => {
    const item = version();
    item.spec.knowledge = { documentCount: 209, sourceCount: 209, chunkCount: 20017,
      sourceIndexId: 'index-311m', snapshotSha256: 'b'.repeat(64),
      profile: { mode: 'hybrid', topK: 16, contextChars: 16000 },
      workflow: { kind: 'adaptive_research', version: 1, maxToolCalls: 6, maxSearchCalls: 3, perQueryTopK: 6 } };
    mount(new MockControlTransport({ routes: { 'agent.eval-lab.apps.get': () => read(app(), item) } }));
    fireEvent.click(await screen.findByText('运行配置与环境要求'));
    expect(await screen.findByText(/最多 6 次工具调用，其中最多 3 次搜索/u)).toBeVisible();
    expect(screen.getByText(/独立页面需要连接持有此冻结版本的 PAW 网关/u)).toBeVisible();
    expect(screen.queryByText(/undefined|先调用模型规划最多/u)).not.toBeInTheDocument();
    expect(screen.queryByText(/独立 App 使用 Python 标准库运行/u)).not.toBeInTheDocument();
  });

  it('reads a citation from the mounted frozen App version without issuing an invocation', async () => {
    const requested: ControlRequest[] = [];
    const item = { ...version(), version: 2 };
    const citation = { sourceId: 'paper', chunkId: 'chunk-9', snapshotSha256: 'b'.repeat(64), page: 9, ordinal: 8 };
    const source = { kind: 'extracted_source', source: { sourceId: 'paper', title: 'Paper' }, citation,
      chunks: [{ chunkId: 'chunk-9', ordinal: 8, page: 9, text: 'Saved evidence.', isCited: true }],
      offset: 8, totalChunks: 9, nextOffset: null, originalUrl: null };
    const transport = new MockControlTransport({ routes: { 'agent.eval-lab.apps.get': (request: ControlRequest) => {
      requested.push(request); return { ok: true, appId: firstId, version: 2, contentHash: item.contentHash, source };
    } } });
    mount(transport, <LabAppPreview app={{ ...app(), latestVersion: 3 }} version={item} calls={[]} onActivity={() => undefined} />);
    const frame = screen.getByTitle('售后助手 · 应用预览') as HTMLIFrameElement;
    const posted = vi.spyOn(frame.contentWindow!, 'postMessage');
    const data = { kind: 'paw.lab-app.source', requestId: 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', locator: citation };
    ...
    expect(requested[0]).toMatchObject({ pathId: 'agent.eval-lab.apps.get', query: {
      appId: firstId, version: 2, sourceId: 'paper', chunkId: 'chunk-9', snapshotSha256: 'b'.repeat(64),
    } });
    await waitFor(() => expect(posted).toHaveBeenCalledWith(expect.objectContaining({ ok: true, result: source }), '*'));
  });
  ...
  it('keeps a prepared version separate from activation and replays an unknown activation exactly', async () => {
    ...
    expect(screen.getByRole('button', { name: '打开应用' })).toBeVisible();
  });
  ...
  it('keeps partial progress text visible in the call receipt when settlement has no answer text', () => {
    const transport = new MockControlTransport({ routes: {} });
    const call: LabAppCall = { callId: 'call-1', appId: firstId, version: 1, actionId: 'answer', input: {}, state: 'interrupted', sessionId: 'session-1', result: {}, error: '连接中断',
      progress: { stage: 'answering', text: '已输出的部分内容' }, cancelRequested: false, createdAtMs: 1, updatedAtMs: 2 };
    mount(transport, <LabAppPreview app={app()} version={version()} calls={[call]} onActivity={() => undefined} />);
    fireEvent.click(screen.getByText(/实际调用记录/u));
    expect(screen.getByText('已输出的部分内容')).toBeVisible();
    expect(screen.getByText('以上为未完成进度文本，最终回执没有提供可用回答。')).toBeVisible();
  });
  ...
  it('delivers sequential settled calls to their own iframe requests', async () => {
    let update!: (calls: LabAppCall[]) => void;
    let callNumber = 0;
    const transport = new MockControlTransport({ routes: { 'agent.eval-lab.apps.command': ({ body }: ControlRequest) => ({
      ok: true, app: app(), call: { callId: `call-${++callNumber}` },
      clientRequestId: (body as Record<string, unknown>).clientRequestId, replayed: false,
    }) } });
    function Surface() {
      const [calls, setCalls] = useState<LabAppCall[]>([]); update = setCalls;
      return <LabAppPreview app={app()} version={version()} calls={calls} onActivity={() => undefined} />;
    }
    mount(transport, <Surface />);
    const frame = screen.getByTitle('售后助手 · 应用预览') as HTMLIFrameElement;
    const notify = vi.spyOn(frame.contentWindow!, 'postMessage');
    const firstRequest = '11111111-1111-1111-1111-111111111111';
    const secondRequest = '22222222-2222-2222-2222-222222222222';
    const send = (requestId: string, question: string) => fireEvent(window, new MessageEvent('message', { source: frame.contentWindow,
      data: { kind: 'paw.lab-app.invoke', requestId, actionId: 'answer', input: { question } } }));
    const settled = (callId: string, question: string, text: string, state: LabAppCall['state']): LabAppCall => ({
      callId, appId: firstId, version: 1, actionId: 'answer', input: { question }, state, sessionId: 'session-1', result: state === 'completed' ? { text } : {},
      error: '', cancelRequested: false, createdAtMs: 1, updatedAtMs: 2,
    });

    send(firstRequest, 'first');
    await waitFor(() => expect(callNumber).toBe(1));
    act(() => update([settled('call-1', 'first', 'answer first', 'running')]));
    act(() => update([settled('call-1', 'first', 'answer first', 'completed')]));
    send(secondRequest, 'second');
    await waitFor(() => expect(callNumber).toBe(2));
    act(() => update([settled('call-1', 'first', 'answer first', 'completed'), settled('call-2', 'second', 'answer second', 'running')]));
    act(() => update([settled('call-1', 'first', 'answer first', 'completed'), settled('call-2', 'second', 'answer second', 'completed')]));

    await waitFor(() => expect(notify).toHaveBeenCalledWith(expect.objectContaining({ kind: 'paw.lab-app.result', requestId: secondRequest,
      result: { text: 'answer second' } }), '*'));
  });
