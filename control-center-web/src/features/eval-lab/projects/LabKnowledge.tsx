...
import { activeKnowledgeJob, defaultCleaning, defaultRetrieval, embeddingModelLabel, type CleaningConfig, type KnowledgeEvaluation, type KnowledgeJob, type RetrievalProfile } from './knowledge-types';
import { CleaningFields, LabKnowledgeIntake } from './LabKnowledgeIntake';
import './lab-knowledge.css';
import { useProjectKnowledge } from './use-project-knowledge';
import { useKnowledgeBaseline } from './knowledge-baseline';
import { parseTrialReceipt } from '../trials/api';

type Page = 'sources' | 'index' | 'evaluation';
type Draft = { baselineRevision?: number; page: Page; corpusId: string; datasetId: string; indexId: string; sourcePath: string; datasetPath: string;
  query: string; strategy: string; size: number; overlap: number; embedding: string; count: number; profile: RetrievalProfile;
  corpusFields: Record<string, string>; datasetFields: Record<string, string>; noDataset: boolean; cleaning: CleaningConfig };
const initial: Draft = { page: 'sources', corpusId: '', datasetId: '', indexId: '', sourcePath: '', datasetPath: '', query: '',
  ...
  corpusFields: {}, datasetFields: {}, noDataset: false, cleaning: defaultCleaning };
const labels: Record<string, string> = { download_wix: '下载 WixQA', import_corpus: '整理资料', connect_base: '连接知识库',
  import_dataset: '导入评测集', import_golden_dataset: '复用冻结评测题', inspect_document: '检查论文', reclean_corpus: '重新清洗', repair_document: '修复论文', reparse_document: '重新解析论文', index: '建立索引', search: '试检索', evaluate: '检索评测' };
...
const chunkingStrategyLabels: Record<string, string> = {
  fixed: '固定长度', paper: '论文结构', markdown: 'Markdown 结构', general: '段落结构', qa: '问答结构',
};
const chunkingStrategyLabel = (strategy: string) => chunkingStrategyLabels[strategy] ?? (strategy || '未知策略');
...
export function LabKnowledge({ project, busy, onCommand, onBind, onOpenBinding, initialPage, initialJobId='',initialBaseline=false,initialCorpusId }: {
  initialPage?: Page; initialJobId?:string;initialBaseline?:boolean;initialCorpusId?:string;
  ...
  const isOffline=transport.kind==='mock' && /^lab-(?:showcase|project-showcase)-/.test(project.projectId);
  const key = `paw.lab.knowledge-draft.v1:${connection}:${project.projectId}`;
  const [draft, setDraft] = useState<Draft>(() => { try { const saved = object(JSON.parse(sessionStorage.getItem(key) ?? '{}')); return { ...initial, ...saved, ...(initialPage ? { page: initialPage } : {}), profile: { ...defaultRetrieval, ...object(saved.profile) }, cleaning: { ...defaultCleaning, ...object(saved.cleaning) } } as Draft; } catch { return { ...initial, ...(initialPage ? { page: initialPage } : {}) }; } });
  const [sourceFiles, setSourceFiles] = useState<File[]>([]); const sourceFile=sourceFiles[0]; const [datasetFile, setDatasetFile] = useState<File>();
  ...
  const knowledgeBindings = project.bindings.filter((row) => row.adapterId === 'golden.knowledge_qa' && row.ownerRef.kind === 'golden_suite');
  ...
  const query = useProjectKnowledge(project.projectId), baseline=useKnowledgeBaseline(project);
  const focusedJob=useQuery({queryKey:['lab-knowledge-focus',connection,project.projectId,initialJobId],enabled:Boolean(initialJobId),retry:false,
    queryFn:async({signal})=>{const job=parseTrialReceipt(await requestLabControl(transport,{pathId:'agent.eval-lab.trials.get',query:{jobId:initialJobId},signal}));if(job.jobId!==initialJobId || job.publicSpec.projectId!==project.projectId || job.sceneId!=='knowledge-resource')throw new Error('此任务不属于当前项目。');return job;}});
  const appliedFocus=useRef(''),appliedBaseline=useRef(false),appliedCorpus=useRef('');
  useEffect(()=>{
    const job=focusedJob.data,state=query.data;
    if(!job || !state || appliedFocus.current===job.jobId) return;
    const result=object(job.result),spec=job.publicSpec;
    const index=state.indexes.find(row=>row.jobId===(spec.indexId ?? (result.kind==='index' ? job.jobId : '')));
    const corpusId=result.kind==='corpus' ? job.jobId : index?.corpusId ?? (typeof spec.corpusId==='string' ? spec.corpusId : undefined);
    patch({...(corpusId ? {corpusId} : {}),...(index ? {indexId:index.jobId,strategy:index.chunking.strategy,size:index.chunking.size,overlap:index.chunking.overlap,embedding:index.dense.provider.semantic ? 'configured' : 'none'} : {}),...(spec.profile ? {profile:{...defaultRetrieval,...object(spec.profile)} as RetrievalProfile} : {}),...(typeof spec.datasetId==='string' ? {datasetId:spec.datasetId} : {})});
    appliedFocus.current=job.jobId;
  },[focusedJob.data,query.data]);
  useEffect(()=>{if(!initialCorpusId || initialJobId || initialBaseline || appliedCorpus.current===initialCorpusId)return;patch({corpusId:initialCorpusId});appliedCorpus.current=initialCorpusId;},[initialCorpusId,initialJobId,initialBaseline]);
  useEffect(()=>{
    const plan=baseline.plan, revision=baseline.summary?.revision;
    if(initialJobId || initialCorpusId && !initialBaseline || !plan || draft.baselineRevision===revision && (!initialBaseline || appliedBaseline.current)) return;
    appliedBaseline.current=true;
    patch({baselineRevision:revision,corpusId:plan.corpusId,indexId:plan.indexId,profile:{...plan.profile},cleaning:{...plan.cleaning},strategy:plan.chunking.strategy,size:plan.chunking.size,overlap:plan.chunking.overlap,embedding:plan.embedding});
  },[baseline.plan,baseline.summary?.revision,draft.baselineRevision,initialJobId,initialBaseline,initialCorpusId]);
  ...
  const indexes = state?.indexes.filter((row) => row.corpusHash === corpus?.corpusHash) ?? [];
  const index = indexes.find((row) => row.jobId === draft.indexId) ?? (baseline.plan && !draft.indexId ? undefined : indexes[0]);
  ...
  useEffect(() => {
    ...
    if (result?.kind === 'corpus') patch({ corpusId: selectedJob.jobId, indexId: '', datasetId: '', page: object(result.intake).status === 'needs_review' ? 'sources' : 'index' });
  ...
  const upload = async (file: File) => {
    ...
    const started = await onCommand({ operation: 'upload_begin', name: knowledgeFileName(file), bytes: file.size, sha256: hash });
  ...
  const importFile = async (kind: 'corpus' | 'dataset') => {
    ...
    try {
      ...
      if(kind==='corpus' && sourceFiles.length>1){const uploadIds:string[]=[];for(const file of sourceFiles)uploadIds.push(await upload(file));source.uploadIds=uploadIds;}
      else Object.assign(source,file ? {uploadId:await upload(file)} : {path:kind==='corpus'?draft.sourcePath:draft.datasetPath});
      await command({ operation: kind === 'corpus' ? 'import_corpus' : 'import_dataset', ...source,
        ...(kind === 'dataset' ? { corpusId: corpus!.jobId } : { cleaning: { ...draft.cleaning } }), fields: kind === 'corpus' ? draft.corpusFields : draft.datasetFields });
    } catch (reason) { setError(projectError(reason)); }
    finally { setUploadProgress(''); }
  };
  const repair = async (sourceId: string, file: File) => {
    setError(''); uploadStop.current = false;
    try { await command({ operation: 'repair_document', corpusId: corpus!.jobId, sourceId, uploadId: await upload(file) }); }
    catch (reason) { setError(projectError(reason)); }
    finally { setUploadProgress(''); }
  };
  ...
  return <section className="lab-knowledge" aria-label="知识库实验">
    <header><div><h2>知识库实验</h2></div><Button size="small" disabled={query.isFetching} onClick={() => void query.refetch()}><RefreshCw size={14} />重新读取</Button></header>
    ...
    {focusedJob.isError ? <p role="alert">{projectError(focusedJob.error)}</p> : focusedJob.data ? <details className="lab-knowledge-focus"><summary>当前实验 · {labels[String(focusedJob.data.publicSpec.operation)]} · {new Date(focusedJob.data.createdAtMs).toLocaleString('zh-CN')} · {stateLabels[focusedJob.data.state]}</summary><pre>{JSON.stringify({configuration:focusedJob.data.publicSpec,result:focusedJob.data.result,error:focusedJob.data.error},null,2)}</pre></details> : null}
    ...
    {!active.length && selectedJob && ['failed', 'interrupted', 'cancelled'].includes(selectedJob.state) ? <p role="status" className="lab-knowledge-status">{labels[selectedJob.publicSpec.operation]} · {stateLabels[selectedJob.state]}。{selectedJob.result?.message || '检查来源或配置后可重新执行。'}</p> : null}
    {draft.page === 'sources' ? <div className="lab-knowledge-section">
      <details className="lab-knowledge-optional" open={!corpus}><summary>{corpus ? '添加或连接更多资料' : '接入第一批资料'}</summary><h3>导入资料</h3><p>{isOffline?'选择多个 PDF、文本或一个文件夹，在当前浏览器中解析并保存。':'上传 PDF、文本或 JSONL 语料，或填写执行器上的文件夹路径。资料会保存为此项目的独立快照。'}</p>
      <div className="lab-knowledge-inline"><label className="lab-project-file-picker"><Upload size={15} />{sourceFiles.length>1 ? `已选择 ${sourceFiles.length} 个文件` : sourceFile?.name ?? '选择语料文件'}<input multiple type="file" accept=".pdf,.txt,.md,.markdown,.html,.htm,.jsonl,.ndjson" disabled={disabled} onChange={(event) => setSourceFiles(Array.from(event.target.files ?? []))} /></label><label className="lab-project-file-picker">选择文件夹<input type="file" multiple ref={element=>{element?.setAttribute('webkitdirectory','');}} disabled={disabled} onChange={event=>setSourceFiles(Array.from(event.target.files ?? []).filter(file=>/\.(pdf|txt|md|markdown|html?|jsonl|ndjson)$/i.test(file.name) && !knowledgeFileName(file).split('/').some(part=>part.startsWith('.'))))}/></label>{sourceFile ? <Button size="small" disabled={disabled} onClick={() => setSourceFiles([])}>移除文件</Button> : null}</div>
      {!sourceFile && !isOffline ? <label>执行器上的文件夹或文件路径<input value={draft.sourcePath} placeholder="例如：/workspace/documents" disabled={disabled} onChange={(event) => patch({ sourcePath: event.target.value })} /></label> : null}
      <p className="lab-knowledge-note">{isOffline?'支持 PDF 文字层、TXT、Markdown、HTML 和 JSONL；单文件最多 100 MB，JSONL 每行一篇。文件在浏览器中处理，不含 OCR。':'原文件总量最多 2 GB，单篇 PDF 最多 100 MB；提取正文最多 300 MB、20,000 篇。'}</p>
      <CleaningFields value={draft.cleaning} disabled={disabled} onChange={(cleaning) => patch({ cleaning })} />
      <FieldMapping kind="corpus" value={draft.corpusFields} onChange={(corpusFields) => patch({ corpusFields })} disabled={disabled} />
      <div className="lab-knowledge-inline"><Button variant="primary" disabled={disabled || (!sourceFile && !draft.sourcePath.trim())} onClick={() => void importFile('corpus')}>整理资料</Button>{!isOffline && <Button disabled={disabled} onClick={() => setShowBases((value) => !value)}>连接已有知识库</Button>}</div>
      <p className="lab-knowledge-note">评测题集请在“评测”页导入。</p>
      ...
      {!corpus ? <Empty onSources={() => patch({ page: 'sources' })} /> : <>
        ...
        <Button variant="primary" disabled={disabled || !Number.isInteger(draft.size) || draft.size < 200 || draft.size > 8000 || draft.overlap < 0 || draft.overlap >= draft.size} onClick={() => void command({ operation: 'index', corpusId: corpus.jobId, chunking: { strategy: draft.strategy, size: draft.size, overlap: draft.overlap }, embedding: draft.embedding })}>建立新索引</Button></details>
        {index ? <><h3>检索测试</h3><label>索引版本<select value={index.jobId} onChange={(event) => patch({ indexId: event.target.value })}>{indexes.map((row, number) => <option key={row.jobId} value={row.jobId}>{indexes.length - number} · {chunkingStrategyLabel(row.chunking.strategy)} {row.chunking.size}/{row.chunking.overlap} · {amount(row.chunkCount)} 片</option>)}</select></label><p>{amount(index.documentCount)} 篇独立正文 / {amount(index.chunkCount)} 个切片 · {index.dense.available ? `${amount(index.dense.vectorCount)} 条向量` : '关键词索引已就绪'}</p>{index.dense.available ? <p className="lab-knowledge-note">当前索引的向量模型：{embeddingModelLabel(index.dense.provider.model) || index.dense.provider.provider}。</p> : null}{index.duplicateSourceCount ? <p className="lab-knowledge-note">保留 {amount(index.sourceCount ?? index.documentCount)} 个原始来源；其中 {amount(index.duplicateSourceCount)} 个来源与其他来源正文完全相同，复用索引并保留引用关系。</p> : null}
          ...
          {dataset?.preview?.length && !draft.query ? <div className="lab-knowledge-examples"><span>题集中的问题</span>{dataset.preview.map((row) => <button key={row.caseId} onClick={() => patch({ query: row.question })}>{row.question}</button>)}</div> : null}
          {searchJob?.result?.hits ? <div className="lab-knowledge-hits"><p>“{String(searchJob.result.query)}” · {searchJob.result.hits.length} 个检索结果</p>{searchJob.result.hits.length ? searchJob.result.hits.map((hit) => <article key={hit.chunkId}><h4>{hit.title}</h4><p>{hit.content}</p><small>来源 {hit.sourceId} · 切片 {hit.chunkId}</small></article>) : <p>未命中资料。可以换一种问法，或调整检索方式后重试。</p>}</div> : null}
      ...
      {!corpus ? <Empty onSources={() => patch({ page: 'sources' })} /> : <>
        <h3>评测题集</h3><details className="lab-knowledge-optional" open={!dataset}><summary>{dataset ? '添加评测题集' : '导入或复用题集'}</summary>
        {knowledgeBindings.length ? <div className="lab-knowledge-existing"><label>复用本项目的回答评测题<select value={suiteId || knowledgeBindings[0]!.ownerRef.id} disabled={disabled} onChange={(event) => setSuiteId(event.target.value)}>{knowledgeBindings.map((row) => <option key={row.bindingId} value={row.ownerRef.id}>{row.summary || `知识库回答评测 · ${row.ownerRef.id.slice(-8)}`}</option>)}</select></label><p className="lab-knowledge-note">先在回答评测中核对并冻结标准，再复用同一批问题、参考答案和开发／保留分组，比较检索配置。</p><Button disabled={disabled} onClick={() => void command({ operation: 'import_golden_dataset', corpusId: corpus.jobId, suiteId: suiteId || knowledgeBindings[0]!.ownerRef.id })}>复用已冻结的评测题</Button></div> : null}
        ...
        <Button disabled={disabled || (!datasetFile && !draft.datasetPath.trim())} onClick={() => void importFile('dataset')}>导入评测集</Button></details>
        ...
        {index && dataset ? <><h3>检索评测</h3><label>本次评测的索引<select value={index.jobId} disabled={disabled} onChange={(event) => patch({ indexId: event.target.value })}>{indexes.map((row, number) => <option key={row.jobId} value={row.jobId}>{indexes.length - number} · {chunkingStrategyLabel(row.chunking.strategy)} {row.chunking.size}/{row.chunking.overlap} · {amount(row.chunkCount)} 片</option>)}</select></label><RetrievalFields value={profile} onChange={updateProfile} disabled={disabled} semantic={index.dense.provider.semantic} reranker={index.reranker.configured === true} />
        ...
        {index ? <><h3>回答评测</h3>{dataset ? <label className="lab-knowledge-check"><input type="checkbox" checked={draft.noDataset} onChange={(event) => patch({ noDataset: event.target.checked })} />本次另起草合成标准，不使用已导入原题</label> : null}
          <label>本轮回答评测题数<input type="number" min={2} max={100} value={draft.count} onChange={(event) => patch({ count: Number(event.target.value) })} /></label><p className="lab-knowledge-note">{dataset && !draft.noDataset ? `从 ${dataset.caseCount} 道原题中稳定选取，并保留开发与验证分组。` : '从知识库的文档样本起草，所有标准先进入待审核状态。'}完整知识库仍参与每次检索。后续起草、评审和回答会调用所选模型。</p>
          ...
          <p className="lab-knowledge-note">下一步：核对评测标准与运行配置。</p></> : null}
      </>}
    </div>}
...
function Empty({ onSources }: { onSources: () => void }) { return <div className="lab-knowledge-empty"><p>尚未导入资料。</p><Button onClick={onSources}>接入资料</Button></div>; }
...
function EvaluationTable({ evaluations, disabled, onUse }: { evaluations: KnowledgeEvaluation[]; disabled: boolean; onUse: (row: KnowledgeEvaluation) => void }) {
  return <div className="lab-knowledge-table"><table><caption>同一冻结评测集的实际检索结果。按参考来源计分：Precision@K = 命中参考来源数 / K，Recall@K = 命中参考来源数 / 参考来源数；F1 先逐题计算再取平均。</caption><thead><tr><th>配置</th><th>分组</th><th>计分 / 计划</th><th>Precision@K</th><th>Recall@K</th><th>F1@K</th><th>MRR</th><th>nDCG@K</th><th>平均耗时</th><th>下一步</th></tr></thead><tbody>{evaluations.map((row) => {
    const metrics = row.report.metrics.metrics; const k = String(row.profile.topK);
    const embedding = row.indexConfig?.embedding;
    const modelName = embeddingModelLabel(embedding?.modelReference || embedding?.model);
    return <tr key={row.jobId}><th>{modeLabels[row.profile.mode]} · K={k}{row.profile.rerank ? ' · 重排' : ''}<small>{row.indexConfig ? `切片：${chunkingStrategyLabel(row.indexConfig.chunking.strategy)} · ${row.indexConfig.chunking.size}/${row.indexConfig.chunking.overlap}` : `索引 ${row.indexId.slice(-6)}`}</small>{embedding && embedding.provider !== 'none' ? <small className="lab-knowledge-model">向量：{modelName || embedding.provider}{embedding.dimensions ? ` · ${embedding.dimensions} 维` : ''}</small> : null}</th><td>{row.split === 'development' ? '开发' : `保留 · 第 ${row.holdoutUseNumber} 次`}</td><td>{row.evaluatedCount} / {row.plannedCount}{row.unlabeledCount ? <small>{row.unlabeledCount} 题缺来源标注</small> : null}</td><td>{rate(metrics.precisionAtK?.[k])}</td><td>{rate(metrics.recallAtK[k])}</td><td>{rate(metrics.f1AtK?.[k])}</td><td>{typeof metrics.mrr === 'number' ? metrics.mrr.toFixed(4) : '—'}</td><td>{rate(metrics.ndcgAtK[k])}</td><td>{row.report.costs.meanRetrievalLatencyMs.toFixed(1)} ms</td><td><Button size="small" disabled={disabled} onClick={() => onUse(row)}>用于回答评测</Button></td></tr>;
  })}</tbody></table></div>;
}
