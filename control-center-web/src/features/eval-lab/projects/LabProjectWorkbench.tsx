...
import { LabPage04Controls } from '@/paw-os/showcase/LabPage04Controls';
...
export function LabProjectWorkbench({ initialProjectId = '', initialProjectPage, initialView, onOpenHistory, onProjectSelect }: { initialProjectId?: string; initialProjectPage?: ProjectPage; initialView?:Partial<ProjectView>; onOpenHistory?: () => void; onProjectSelect?: (id: string,view?:ProjectView) => void }) {
  ...
  const page04Flow = transport.kind === 'mock' && /^lab-page04-(rag|enterpriseops|cloudops|memory)$/.test(projectId);
  ...
  const projectDefaultView: ProjectView = { ...defaultProjectView, ...(demoFlow ? { page:'materials', journeyStep:0 } : page04Flow || transport.kind==='mock' && (projectId==='lab-showcase-rag' || projectId.startsWith('lab-project-showcase-')) ? {page:'journey',journeyStep:0} : {}), guideOpen:project?.workspace.layout !== 'focus' };
  ...
  return <main className="eval-lab lab-project-workbench" aria-label="Agent Lab 项目工作台">
    ...
    {page04Flow && project ? <LabPage04Controls project={project} busy={busy} status={object(object(workflow.project.data).page04)} onSubmit={async (action,input) => { const receipt=await workflow.submit(action,input,project); if(receipt) await workflow.project.refetch(); return receipt; }} onNavigate={updateView}/> : null}
