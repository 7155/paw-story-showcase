...
export function LabProjectWorkbench({ initialProjectId = '', onOpenHistory, onProjectSelect }: { initialProjectId?: string; onOpenHistory?: () => void; onProjectSelect?: (id: string) => void }) {
  ...
  const demoFlow = transport.kind === 'mock' && /^lab-showcase-(enterpriseops|rag|cloudops|memory)$/.test(projectId);
  const flowInitialized = useRef('');
  ...
  useEffect(() => {
    if (!demoFlow || !project || flowInitialized.current === projectId) return;
    flowInitialized.current = projectId;
    if (!project.materialCount) updateView({ page: 'materials', guideOpen: false, artifactId: 'demo-intake' });
  }, [demoFlow, project, projectId, updateView]);
        ...
        <div className="lab-project-results">
          ...
          <div className="lab-project-stage">
            {page === 'materials' ? <Materials project={project} onAdd={() => setIntakeOpen(true)} />
              ...
              : page === 'apps' ? <LabAppDelivery key={demoFlow ? `${project.projectId}:${object(object(workflow.project.data).demoFlow).appVersion ?? 0}` : undefined} projectId={project.projectId} preparing={busy} onPrepare={async (directory, appId) => Boolean(await workflow.submit('prepare_app', { directory, ...(appId ? { appId } : {}) }, project))} />
