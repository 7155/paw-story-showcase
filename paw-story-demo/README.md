...
当前主入口 `/` 是分阶段的真实前端工作区，`/?view=full` 保持兼容。固定导航一次聚焦一个模块；Room、独立 Trace App、Agent Lab 和 Memory 嵌入 `control-center-web` 的实际组件；输入章节保留输入法交互演示。紫色描边指向真实控件，观察到普通点击及其界面结果后才推进引导。引导通过真实控件点击和界面结果推进。所有详情页也先呈现真实工作区，技术说明与历史示意默认折叠。
