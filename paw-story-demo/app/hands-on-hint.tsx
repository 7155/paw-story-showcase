...
export function HandsOnHint({ frame, source, opaque = false }: { frame: RefObject<HTMLIFrameElement | null>; source?: string; opaque?: boolean }) {
  ...
  useEffect(() => {
    ...
    let visible = false;
    const sendControl = () => element?.contentWindow?.postMessage({ channel: 'paw.hands-on-control', instanceId: instance, playing: playing && visible && !document.hidden }, opaque ? '*' : url.origin);
    const observer = new IntersectionObserver(entries => { visible = entries[0]?.isIntersecting === true && !element?.closest('[hidden]'); sendControl(); }, { threshold: 0.15 });
    if (element) observer.observe(element);
    const timer = window.setInterval(sendControl, 500);
    const onMessage = (event: MessageEvent) => {
      const data = event.data;
      if (event.source !== element?.contentWindow || event.origin !== (opaque ? 'null' : url.origin) || !data || data.channel !== 'paw.hands-on' || data.version !== 1 || data.instanceId !== instance) return;
    ...
    return () => { clearInterval(timer); observer.disconnect(); element?.contentWindow?.postMessage({ channel: 'paw.hands-on-control', instanceId: instance, playing: false }, opaque ? '*' : url.origin); window.removeEventListener('message', onMessage); };
  }, [frame, source, playing, opaque]);
